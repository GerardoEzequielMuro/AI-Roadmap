...
const DIDACTICO: Record<string, Didac> = {
  ...
  'f1-1': {
    concepto: 'Aprender, para una máquina, es puro juego de frío-caliente. La loss es un número que mide cuánto le erra el modelo: cuanto más alto, peor. El gradiente es la brújula que dice hacia dónde mover cada parámetro para que ese número baje, y el gradient descent es dar pasitos en esa dirección una y otra vez. Pensalo como bajar un cerro con niebla: no ves el fondo, pero sentís la pendiente bajo los pies y das un paso cuesta abajo; repetís hasta que el piso se aplana.',
  ...
  'f3-3': {
    concepto: "Attention es un mecanismo de búsqueda dentro de la propia oración. Cada token emite una Query ('qué ando buscando'), y todos los demás ofrecen una Key ('qué tengo para ofrecer'); el producto entre Query y Key dice cuánto le presta atención uno a otro, y con esos pesos mezcla los Value ('qué información traigo'). Así, en 'el gato que perseguía al ratón estaba cansado', el token 'cansado' puede mirar fuerte a 'gato' aunque estén lejos, y resolver de quién habla. Eso es lo que las RNN hacían mal por procesar en fila india: self-attention deja que cada token mire a todos de una, y esa es la bisagra de toda la era actual de la IA.",
